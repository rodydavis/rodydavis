...
async function processFile(file, force, current, total) {
  try {
    ...
    // Check modification time relative to last metadata update (with a 2-second threshold for OS file-write delay)
    const stat = await fs.stat(file);
    const lastUpdated = parsed.data.last_updated_metadata ? new Date(parsed.data.last_updated_metadata) : null;
    const isModified = lastUpdated ? (stat.mtime - lastUpdated > 2000) : true;

    if (!!force || !parsed.data.summary || isModified) {
      const summary = await generateSummary(parsed.content.slice(0, 2000));
      if (summary) {
        parsed.data.summary = summary;
        needsUpdate = true;
        console.log(`${prefix} Generated/updated summary for: ${relativePath}`);
      }
    }
    ...
    if (
      ...
      Array.isArray(parsed.data.embedding) ||
      isModified
    ) {
      const embedding = await generateEmbedding(
        parsed.data.summary || parsed.content.slice(0, 2000),
      );
      if (embedding) {
        parsed.data.embedding = embedding;
        needsUpdate = true;
        console.log(`${prefix} Generated/updated embedding for: ${relativePath}`);
      }
    }

    if (needsUpdate) {
      parsed.data.last_updated_metadata = new Date().toISOString();
...
async function processFiles({ force, updateTags }) {
  ...
  if (existsSync(TAGS_FILE)) {
    taxonomy = readFileSync(TAGS_FILE, "utf8")
      .split("\n")
      .map((t) => t.trim())
      .filter((t) => t.length > 0);
  ...
  if (taxonomy.length > 0) {
    ...
    await asyncPool(CONCURRENCY_LIMIT, fileData, async (item) => {
      ...
      const tagsChanged =
        JSON.stringify(currentTags) !== JSON.stringify(tagsToSet);
      ...
      if (tagsChanged) {
        ...
        console.log(
          `  [${count3}/${fileData.length}] Updated tags for ${path.relative(CONTENT_DIR, item.file)}: ${newTags.join(", ")}`,
        );
  ...
  await asyncPool(
    CONCURRENCY_LIMIT,
    Array.from(embeddingsMap.entries()),
    async ([relativePath, embedding]) => {
      count4++;
      try {
        ...
        const isDifferent =
          relatedPaths.length !== currentRelated.length ||
        ...
        if (isDifferent || force) {
          ...
          console.log(
            `  [${count4}/${embeddingsMap.size}] Updated related posts for: ${relativePath}`,
          );
        } else if (count4 % 20 === 0 || count4 === embeddingsMap.size) {
          console.log(
            `  Progress: ${count4}/${embeddingsMap.size} related check complete`,
          );
        }
      } catch (err) {
