...
export async function writeRssFile(path: string): Promise<string> {
  ...
  for await (const dirent of dir) {
    ...
    if (meta !== "") {
      ...
      for (const line of lines) {
        ...
        if (line.match("date")) {
          post.date = new Date(Date.parse(line.split(": ")[1].trim()));
        }
      }
      ...
      post.image = xmlescape(`https://rodydavis.com${post.image}`);
      feed.addItem(post);
    }
  }
